...
public class Board {
    ...
    public boolean checkForFoundItem(Item item) {
        ...
        Piece[][] chunk = new Piece[shape.length][shape[0].length];
        boolean isItemFound = true;
        // Extract chunk of shape from board
        ...
        // Compare the shape's 1 values to the corresponding FoundITEM coordinates
        for (int i = 0; i < shape.length; i++){
            for (int j = 0; j < shape[0].length; j++){
                if (shape[i][j] == 1 && chunk[i][j] != Piece.FOUNDITEM){
                    isItemFound = false;
                }
            }
        }
