...
public class GameViewer implements MouseListener {
    ...
    public int[] getNearestPiece(int xClicked, int yClicked) {
        // TODO 36
        //pixel coord to array index conversion
        int xCoord = xClicked / bkSize;
        int yCoord = yClicked / bkSize;
    ...
    public void drawGameOutcome() {
        // TODO 39
        numberOfFoundItems = bd.getFoundItemsList(selectedItems).size();
        if (numberOfFoundItems == numberOfHiddenItems){
            sc.drawGameWon(0, 0);
        }
    ...
    public int takeTurn(int x, int y) {
        // TODO 41
        if (getTurnsRemaining() > 0){
            reduceTurns(bd.searchSpace(x, y));
            refreshBoard(x, y);
        }
        return getTurnsRemaining();
    }
    ...
    public void mousePressed(MouseEvent e) {
        // TODO 42
        int[] coord = getNearestPiece(e.getX(), e.getY());
        
        //xCheck and yCheck mark the boundary for the restart button location on the display
        boolean xCheck = e.getX() >= 0 && e.getX() <= 150;
        boolean yCheck = e.getY() >= brdSize + scoreboardSize - 50 && e.getY() <= brdSize + scoreboardSize;
        if (xCheck && yCheck){
            restartGame();
        }
        else{
